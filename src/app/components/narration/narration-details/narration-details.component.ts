...
@Component({
  selector: 'app-narration-details',
  templateUrl: './narration-details.component.html',
  styleUrls: ['./narration-details.component.scss']
})
export class NarrationDetailsComponent implements OnInit {
  // id of item to view
  @Input() itemId = '';
  ...
  deleteItem() {
    ...
    const dialogRef = this.dialog.open(ConfirmDeleteNarrationDialog, {
      width: '250px',
    ...
    dialogRef.afterClosed().subscribe((results) => {
      if (results == true) {
        this._api.deleteTypeRequest('narrations/' + this.itemId).subscribe((res:any) => {
          if (res.message == 'Narration was deleted successfully!') {
            this._snackBar.open('Item successfully deleted!', '', { duration: 3000 });
          }
          else {
            this._snackBar.open('Problem deleting item!', '', { duration: 5000 });
          }
          this._router.navigate(['/narrations']);
        });
      }
    });
    return true;
...
@Component({
selector: 'confirm-delete-narration-dialog',
templateUrl: 'confirm-delete-narration.dialogue.html',
})
export class ConfirmDeleteNarrationDialog {
  constructor(
    public dialogRef: MatDialogRef<ConfirmDeleteNarrationDialog>,
