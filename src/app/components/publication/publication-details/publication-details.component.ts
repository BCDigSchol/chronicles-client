...
export class PublicationDetailsComponent implements OnInit {
  ...
  deleteItem() {
    ...
    const dialogRef = this.dialog.open(ConfirmDeletePublicationDialog, {
      width: '250px',
      data: { id: this.itemId, title: this.protectedData.title },
    });
    // alter user role after closed, if confirmed
    dialogRef.afterClosed().subscribe((results) => {
      if (results == true) {
        this._api.deleteTypeRequest('publications/' + this.itemId).subscribe((res:any) => {
          if (res.message == 'Publication was deleted successfully!') {
...
@Component({
  selector: 'confirm-delete-publication-dialog',
  templateUrl: 'confirm-delete-publication.dialogue.html',
})
export class ConfirmDeletePublicationDialog {

  constructor(
    public dialogRef: MatDialogRef<ConfirmDeletePublicationDialog>,
