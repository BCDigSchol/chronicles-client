...
export class AuthorDetailsComponent implements OnInit {
   ...
   */
  update() {
    this._api.getTypeRequest('authors/' + this.itemId).subscribe({
      next: (res: any) => {
  ...
  deleteItem() {
    ...
    const dialogRef = this.dialog.open(ConfirmDeleteAuthorDialog, {
      width: '250px',
      data: {id: this.itemId, title: this.protectedData.surname + ', ' + this.protectedData.otherNames},
    });
    // alter user role after closed, if confirmed
    dialogRef.afterClosed().subscribe((results) => {
      if (results == true) {
        this._api.deleteTypeRequest('authors/' + this.itemId).subscribe((res:any) => {
          if (res.message == 'Author was deleted successfully!') {
...
@Component({
  selector: 'confirm-delete-author-dialog',
  templateUrl: 'confirm-delete-author.dialogue.html',
})
export class ConfirmDeleteAuthorDialog {

  constructor(
    public dialogRef: MatDialogRef<ConfirmDeleteAuthorDialog>,
