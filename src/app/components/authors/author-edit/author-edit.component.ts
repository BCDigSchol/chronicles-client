...
export class AuthorEditComponent implements OnInit {
  ...
  // for store current/possible items for select boxes for gender, nationality, specific nationality, etc
  // for storing possible and current values for drop down selection menus
  selectionOptions: SelectionOptions = {
    gender: ['Female', 'Male', 'Uncertain or N/A'],
    nationality: ['British', 'Irish', 'American', 'French', 'German', 'Australian', 'Indian', 'South African', 'Other European', 'Other Empire', 'Other'],
    specificNationality: ['', 'English', 'Scottish', 'Welsh', 'Cornish'],
    nonPerson: [true, false],
  };
  ...
  ngOnInit(): void {
    ...
    this._api.getTypeRequest('authors/' + this.itemId).subscribe((res: any) => {
  ...
  private _validate(reqObject: any): boolean {
    ...
    if (reqObject.gender != 'Male' && reqObject.gender != 'Female' && reqObject.gender != 'Uncertain or N/A') {
      this.errorMsgs.push('Gender must be "Male", "Female", or "Uncertain or N/A"');
      isValid = false;
    }
    return isValid;
  }
  ...
  onSubmit(form: any) {
    var reqObject = {
      ...
      surname: this.protectedData.surname,
    ...
    if (this._validate(reqObject)) {
      this._api.putTypeRequest('authors/' + this.protectedData.id.toString(), reqObject).subscribe((res: any) => {
        if (res.status !== 0) {
          this._snackBar.open('Item successfully updated!', '', { duration: 3000 });
          // navigate to conferences
          this._router.navigate(['/authors/' + this.protectedData.id]);
        }
        else {
